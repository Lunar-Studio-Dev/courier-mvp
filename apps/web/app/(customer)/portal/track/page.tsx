...
export default function CustomerTrackPage() {
  ...
  const { data, isLoading, error } = trpc.shipments.track.useQuery(
    ...
    { enabled: !!searchValue, retry: false },
  );
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-2xl font-semibold mb-6">Track Shipment</h1>
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <Input
              ...
              placeholder="Enter tracking number (e.g. TPC...)"
              className="flex-1"
              autoFocus
            />
            ...
            <CardContent className="py-6">
              ...
              <div className="mt-2 flex flex-wrap gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1">
                  <Clock className="h-3 w-3" /> Booked: {formatDate(data.bookedAt)}
                </span>
                {data.deliveredAt && (
                  <span className="flex items-center gap-1">
              ...
              <CardContent>
                <div className="flex items-center justify-between mb-2">
                  {ALL_STATUSES.map((status, i) => {
                    const isCompleted =
                      i <= currentStatusIndex && completedStatuses.includes(status);
